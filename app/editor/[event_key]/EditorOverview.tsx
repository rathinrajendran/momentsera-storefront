...
export default function EditorOverview({ activeTab, currentSections = [], onSelect, scrollTop, onScrollChange, eventKey }: Props) {
  ...
  /**
   * currentSections is the single source of truth for the
   * currently selected tab and its dynamic metadata.
   */
  ...
  return (
    <div className="flex h-auto flex-col overflow-x-auto p-5 px-4 md:h-full md:bg-white md:px-6 [&::-webkit-scrollbar]:h-[0px] [&::-webkit-scrollbar]:w-0 [&::-webkit-scrollbar-thumb]:rounded-md [&::-webkit-scrollbar-thumb]:bg-[#c1c1c1] [&::-webkit-scrollbar-track]:rounded-md [&::-webkit-scrollbar-track]:bg-[#78909C]">
      ...
      <div className="flex-1">
        <div ref={scrollRef} className="h-auto space-y-1 overflow-y-auto md:space-y-1">
          {activeTab === "preview" ? (
...
function EditorCard({
  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1 items-center gap-2 sm:gap-4">
        <div
          className={`${iconBg} ${iconColor} flex h-7 w-7 shrink-0 items-center justify-center rounded-sm border border-zinc-100/50 sm:h-10 sm:w-10 sm:rounded-lg`}
        >
          {icon}
        </div>

        <div className="min-w-0 flex-1">
          <h3 className="truncate font-semibold text-zinc-800 text-[0.5rem]">{title}</h3>
        </div>
      </div>
