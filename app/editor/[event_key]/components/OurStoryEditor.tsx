...
/* ---------------- COMPONENT ---------------- */
export default function OurStoryEditor({ onBack, eventKey }: { onBack: () => void; eventKey: string }) {
  // Hooks
  ...
  return (
    <div className="animate-in fade-in flex h-full flex-col rounded-lg duration-500 md:rounded-none">
      <EditorHeader title="Our Story ourStory" handleCancel={handleCancel} />
      <Form {...form}>
        <form
        ...
        >
          ...
          <div className="sticky bottom-0 z-[99] -m-5 flex h-16 items-center justify-end gap-3 border-slate-100 bg-white/90 px-5 md:border-t md:backdrop-blur-md">
            ...
            {form.formState.errors.ourStory?.message && (
              <p className="px-5 text-xs text-red-500">{form.formState.errors.ourStory.message}</p>
            )}
          </div>
        </form>
      </Form>
