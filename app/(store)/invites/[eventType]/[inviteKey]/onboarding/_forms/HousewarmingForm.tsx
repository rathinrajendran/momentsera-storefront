...
const schema = z.object({
  host_names: z.string().min(1, "Host name(s) required"),
...
export default function HousewarmingForm({ inviteKey }: { inviteKey: string }) {
  ...
  async function onSubmit(values: FormData) {
    ...
    try {
      const result = await submitOnboarding({ invite_key: inviteKey, event_type: "housewarming" }, formPayload);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} placeholder="Alex & Sam" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>New Home Address</FormLabel>
              <FormControl>
                <Input {...field} placeholder="123 Maple Street" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
