...
function PortalButton({ children, onClick, dark = false }: { children: ReactNode; onClick?: () => void; dark?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={`group relative inline-flex items-center gap-5 overflow-hidden border px-3 py-2 text-[9px] tracking-[0.24em] uppercase transition-all duration-500 ${
    ...
    >
      ...
      <span
        className={`relative z-10 flex h-8 w-8 items-center justify-center transition-all duration-500 group-hover:rotate-45 ${
          dark
    ...
    <main className="relative overflow-x-clip bg-[#090a0a] text-white selection:bg-white selection:text-black">
      ...
      <section ref={heroRef} onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)} className="relative h-[300vh]">
        <div className="sticky top-0 h-[calc(100svh-72px)] min-h-[650px] overflow-hidden bg-[#090a0a]">
          ...
          >
            <div className="absolute inset-0 flex items-center justify-center">
              ...
              <motion.div
                ...
                className="absolute bottom-[20%] left-[4%] z-20 hidden border border-white/15 bg-black/35 px-4 py-3 backdrop-blur-xl md:block"
              >
          ...
          >
            <span className="flex h-11 w-11 items-center justify-center border border-white/15 bg-white/[0.04] text-white/60 backdrop-blur-md transition-all hover:border-white/40 hover:bg-white hover:text-black">
