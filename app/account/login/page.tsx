  ...
  const onSubmit = (values: FormData) => {
    loginMutation.mutate(values, {
      onSuccess: (data: any) => {
        ...
        const rawPendingEvent = localStorage.getItem("pending_event");

        if (rawPendingEvent) {
          try {
            const pending = JSON.parse(rawPendingEvent);

            if (pending?.event_type && pending?.invite_key) {
              router.replace(`/invites/${pending.event_type}/${pending.invite_key}/onboarding`);

              return;
            }
          } catch (error) {
            console.error("Invalid pending_event:", error);
          }
        }
